...
export async function up(knex: Knex): Promise<void> {
    await knex.schema.createTable("orders_items", (table) => {
        ...
        table.integer("food_id").unsigned().notNullable().references("id").inTable("menu_foods");
        table.integer("drink_id").unsigned().notNullable().references("id").inTable("menu_drinks");
